...
cart = {
    ...
    addGoods:function(goods){
        ...
        if(ShoppingCart==null||ShoppingCart==""){
            ...
            var jsonstr = {"goodsList":[{"id":goods.id,"cateId":goods.cateId,"name":goods.name,"price":goods.price,
            				...
            				"totalNumber":goods.num,"totalAmount":(parseFloat(goods.discount)*parseFloat(goods.num))};
            utils.setParam("ShoppingCart","'"+JSON.stringify(jsonstr)); //将json对象解析成字符串
        }else{
            var jsonstr = JSON.parse(ShoppingCart.substr(1,ShoppingCart.length));//将字符串解析成json对象
    ...
    updateGoodsNum:function(index,num){
        ...
        for(var i in goodsList){
            if(i==index){
	            console.log(jsonstr.totalAmount);
				console.log(goodsList[i].discount);
...
function addCart(num,size,color){
	...
	var goodsPostalfee = $("#goodsPostalfee").val();
	var pic = $("#pic").html();
